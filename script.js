 ...
 */
 ...
 function clearAll() {
  const clearMemes = document.querySelector('.meme-content');
  const clearJokes = document.querySelector('.joke-content');
  const clearQuotes = document.querySelector('.quote-content');
  const clearRiddle = document.querySelector('.riddle-content');
  clearMemes.innerHTML = '';
  clearJokes.innerHTML = '';
  clearQuotes.innerHTML = '';
  clearRiddle.innerHTML = '';
}
...
function showMeme() {
  ...
  imgTag.setAttribute('src', randomMemeUrl);
  clearAll();
...
function showJoke() {
  ...
  clearAll();
  pTag.textContent = randomJokeText;
...
function showQuote() {
  ...
  clearAll();
  quoteContent.textContent = 'Quote: ' + randomQuote.quote;
  authorContent.textContent = 'Author: ' + randomQuote.author;
...
function showRiddle() {
  ...
  const isRiddle = document.createElement('p');
  isRiddle.setAttribute('id', 'question');
  const isRiddleAnswer = document.createElement('p');
  isRiddleAnswer.setAttribute('id', 'answer');
  clearAll();
  container.appendChild(isRiddle);
  isRiddle.textContent = randomRiddle.question;
  isRiddleAnswer.textContent= randomRiddle.answer;
  container.appendChild(isRiddleAnswer);
...
function revealAnswers() {
  const randomRiddle = getRandomData("riddles");
  const container = document.querySelector('.riddle-content');
  const isRiddleAnswer = document.getElementById('answer');
  const isRiddle = document.getElementById('question');
  if(isRiddle && isRiddleAnswer.hidden == true ){
    isRiddleAnswer.hidden = false;
  }
  else if(isRiddleAnswer.hidden == false){
    window.alert("Answer already revealed");
  }
  if(isRiddle == null){
    window.alert("There is no Riddle");
  }
